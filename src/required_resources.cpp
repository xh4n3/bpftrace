...
int RequiredResources::create_maps_impl(BPFtrace &bpftrace, bool fake)
{
  ...
  if (bpftrace.feature_->has_map_ringbuf())
  {
    ...
    auto rb_loss_cnt = std::make_unique<T>(
        "ringbuf_loss_counter", libbpf::BPF_MAP_TYPE_PERCPU_ARRAY,
        sizeof(bpftrace.rb_loss_cnt_key_), sizeof(bpftrace.rb_loss_cnt_val_), 1, 0);
